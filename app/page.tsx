...
function DeviceDashboard() {
  ...
  const handleNotificationChange = async (
    ...
    Modal.confirm({
      title: `Turn ${checked ? 'on' : 'off'} notification for this device?`,
      okText: 'OK',
      cancelText: 'Cancel',
      onOk: async () => {
        try {
  ...
  const columns = [
    {
      title: 'Screen',
      dataIndex: ['lastHeartbeatDetails', 'screenImagePath'],
      key: 'screenImagePath',
      render: (screenImagePath: string) =>
        screenImagePath ? (
          <Image
            height={150}
            src={`${screenImagePath}?t=${new Date().getTime()}`}
            fallback={DEFAULT_FALLBACK_IMAGE}
          />
        ) : (
          <Image height={150} src='error' fallback={DEFAULT_FALLBACK_IMAGE} />
        ),
    },
    {
      title: 'Status',
    ...
    },
    {
      ...
      key: 'notification',
      render: (notification: boolean, record: any) => (
        <Switch
